/**
@author mistaguy
MALERIA OPENCV MODULE
sorce file for ocula wm
**/
#include "stdafx.h"
#include "DLL.h"
...
CvSeq *findmatches(CvSeq imageDescriptors[],float mu[],float inverse_sigma[],int threshold)
{
 ...
 int desc_num=imageDescriptors->total;
...
//Grab Current Mahalnobis distance
DLL_API int getMatchThreshold()
{
	return MATCH_THRESHOLD;
}
//Get Current Matched Surf features
DLL_API int getMatches()
{
	return MATCHES;
}
//Grab Current Features Detected on the image
DLL_API int getFeatures()
{
	return FEATURES;
}
//set Mahalnobis distance
DLL_API void setMatchThreshold(int t)
{
	MATCH_THRESHOLD=t;
}


 //function to detect maleria parasite
    DLL_API short detectParasite(const char * in_file)
	{

		short msgint=0;
		try
		{
        //create memory for use
		CvMemStorage* storage = cvCreateMemStorage(0);
		//colors to be used
		...
		IplImage* object = cvLoadImage( in_file);
  ...
  static CvScalar colour_red = {{255,0,0}};
	...
	IplImage* img = cvCreateImage(cvGetSize(object), IPL_DEPTH_8U, 1);
	cvCvtColor(object, img, CV_BGR2GRAY);

	IplImage* bigimage=cvCreateImage(cvSize(640,480), IPL_DEPTH_8U, 1);
	cvResize(img,bigimage,0);


	
		
	     cvExtractSURF( bigimage, 0, &objectKeypoints, &objectDescriptors, storage, params );
		 FEATURES=objectKeypoints->total;
		 ...
		 if (sizeof(objectKeypoints)>0){
			...
			for( int i = 0; i <MATCHES; i++ )
            {
                ...
                radius = cvRound(15);
				cvCircle(bigimage, center, radius,colors[6], 2, 8, 0 );
		 ...
		 cvSaveImage("\\Program Files\\ocula\\parasite.jpg",bigimage);
		 cvReleaseImage(&img);
		// cvReleaseImage(&object);
		 //cvReleaseImage(&imcolour);
		 cvReleaseMemStorage(&storage);

		// cvReleaseImage(&bigimage);
}
catch(Exception_info_ptr p){}
		 return msgint;
