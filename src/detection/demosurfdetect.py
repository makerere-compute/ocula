...
if __name__ == '__main__':
    ...
    try:
        ...
        #for i in range(1):
            ...
            #imcolour = cv.LoadImage('/home/jq/dev/ocula/data/200/test.jpg')
            ...
            if len(imageDescriptors)>0:
                ...
                if len(matches)>0:
                    cv.PutText(imcolour, 'parasites detected', (10,20), cv.InitFont(cv.CV_FONT_HERSHEY_SIMPLEX,0.8,0.8), (255,255,255))
            cv.ShowImage("ParaSight", imcolour)
    
            # quit if escape key is pressed
            if cv.WaitKey(10) == 27:
                break
        #cv.SaveImage('result.png',imcolour)
    finally:
        capture = None
